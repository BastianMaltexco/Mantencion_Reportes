...
from app.services.dashboard import (
    DashboardPermissionError, DashboardValidationError, dashboard_summary as build_dashboard_summary,
    parse_filters,
)
...
def dashboard_summary():
    ...
    try:
        ...
        data = build_dashboard_summary(filters, actor_id=g.api_user.id, is_admin=_is_admin(), activity_page=activity_page, activity_page_size=activity_page_size)
        ...
        raise ApiError(403, "insufficient_role", str(error)) from error
    data["activity"] = [{**item, "occurred_at": _datetime_value(item["occurred_at"])} for item in data["activity"]]
    technicians = (db.session.execute(select(User).where(User.is_active == True).order_by(User.full_name)).scalars().all()  # noqa: E712
                   if _is_admin() else [g.api_user])
    data["technicians"] = [_user_payload(user) for user in technicians]
    return jsonify({
        "data": data,
